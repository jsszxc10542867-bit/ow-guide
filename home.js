...
// ---------- 레슨 메타 (배우기 6단계) ----------
const LESSON_META = {
  11: { no: '01', en: 'GAME FUNDAMENTALS', min: 14, diff: 1, headline: '오버워치는 "5명이 모여서 한타에 들어가느냐"의 게임입니다.', objectives: ['조작·HUD·체력 종류를 읽을 수 있다', '승리 조건과 모드별 목표를 이해한다', '판 시작 전 조합을 보고 2가지 질문에 답한다', '체력도 자원임을 안다'] },
  1: { no: '02', en: 'ROLES', min: 18, diff: 1, headline: '탱커는 벽에 붙어 선을 긋고, 딜러는 맵을 넓게 쓰고, 힐러는 팀의 허리입니다.', objectives: ['1-2-2와 역할 패시브를 이해한다', '내 역할의 기본기 3가지를 안다', '메인딜러와 서브딜러의 차이를 안다', '어떤 역할부터 시작할지 정한다'] },
  2: { no: '03', en: 'TACTICAL FUNDAMENTALS', min: 20, diff: 2, headline: '본대는 사이드를 보고, 사이드는 본대를 위해 근거를 만듭니다.', objectives: ['한타의 시작·중간·끝을 구분한다', '본대와 사이드의 순환 규칙을 안다', '강한 턴과 약한 턴을 구분한다', '이겼을 때·졌을 때 할 일을 안다'] },
  3: { no: '04', en: 'POSITIONING', min: 14, diff: 2, headline: '좋은 포지션은 "안 죽는 자리"가 아니라 "선택지가 많은 자리"입니다.', objectives: ['좋은 포지션의 조건을 이해한다', '맵의 고지·코너·시야선을 읽는다', '선을 넘는 조건 3가지로 진입·후퇴를 판단한다'] },
  4: { no: '05', en: 'SURVIVAL & HABITS', min: 11, diff: 2, headline: '죽지 않는 것이 가장 큰 딜입니다.', objectives: ['죽는 원인 5가지를 안다', '증상별로 픽을 바꿀 줄 안다', '판당 데스 목표를 세운다'] },
  5: { no: '06', en: 'ADVANCED STRATEGY', min: 20, diff: 3, headline: '상위 티어는 상대의 자원(궁·쿨·체력)을 보고 싸웁니다.', objectives: ['조합(다이브·러시·포킹)의 승리 조건을 안다', '쿨·궁 트래킹을 시작한다', '카운터픽 판단 기준을 세운다'] }
};
